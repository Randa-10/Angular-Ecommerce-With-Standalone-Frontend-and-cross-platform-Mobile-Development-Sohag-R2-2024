...
export class ProductStaticService {
  ...
  constructor() {
   ...
   getproductById(prdId:number):Iproduct|undefined{
    return this.productsListService.find((id)=>id.productId==prdId)
   }

   deSearchProduct(filterValue:string):Iproduct[]{
    filterValue=filterValue.toLowerCase()
    return  this.productsListService.filter((Prd:Iproduct)=>
  ...
  getAllIds():number[]{
    //[id1,id2.......]
    return this.productsListService.map((id)=>id.productId)
  }
