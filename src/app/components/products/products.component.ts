...
export class ProductsComponent implements OnDestroy, OnInit {
  ...
  ngOnInit(): void {
    ...
    //day6
    //  this.productsApi.getAllProduct().subscribe({
    //   next:(data)=>{
    //     console.log(data);
    ...
    this.sub = this.productsApi.getAllProduct().subscribe({
      next: (data) => {
        console.log(data);
  ...
  @Input() set filterByNameInChild(vaule: string) {
    ...
    this.productsApi.getAllProduct().subscribe((data) => {
      this.productsAfterSearch = data.filter((prd) =>
        prd.productName.toLowerCase().includes(vaule)
      );
    });
  }
  ...
  //day4
  //create event
  @Output() productsInChild: EventEmitter<Iproduct> =
